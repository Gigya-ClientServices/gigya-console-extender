...
function injectStyles() {
	$(document).ready(function()
	{
		...
		originalAppKey = $('#Details > div > div.dialog').html();
		...
		// Add AppKey Copy
		$('#Details > div > div.dialog .float_left').each(function(index){
			var t = $(this).text();
			$(this).html(createCopyText(t));
		});
		...
		$('.site_row').each(function(index) {
			...
			if (isChild) {
				apiKeys[lastParent].children.push(row);
			}
			if (!isChild) {
				apiKeys[c] = row;
				c = c + 1;
...
function removeStyles() {
	$(document).ready(function() {
		...
		$('#Details > div > div.dialog').html(originalAppKey)
...
function createSitesGrid(collection, index) {
	var grid = ''
	var showBoilerplate = false;
	...
	if (showBoilerplate) {
		...
		// var accId = 'accordion' + index;
		// var collapseId = 'collapse' + index;
		grid = grid +
		// 	'<div class="panel-group" id="' + accId + '" role="tablist">' +
		 	'<div class="panel panel-default">' +
		// 	'<div class="panel-heading" role="tab" id="headingOne">' +
    //   '<p class="panel-title">' +
		// 	'<a role="button" data-toggle="collapse" data-parent="#' + accId + '" href="#' + collapseId + '">' +
		// 	'Child Sites' +
    //   '</a>' +
    //   '</p>' +
    // 	'</div>' +
    // 	'<div id="' + collapseId + '" class="panel-collapse collapse" role="tabpanel">' +
 	...
 	grid = grid + '<div class="sites_list row">';
	for (var i = 0, len = collection.length; i < len; i++) {
		var row = collection[i];
		var rowText =
			'<div class="site-row row" style="background-color:'+ (((i+(index%2))%2)==0?'#f0f6fb;':'#ffffff;') + '">' +
			...
			'</div>' +
			((row.children.length > 0)?'<div class="inner-site-list">' + createSitesGrid(row.children, i) + '</div>':'') +
			'</div>' +
			'</div>';
		grid = grid + rowText;
  }
	grid = grid + '</div>';
	if (showBoilerplate) {
		grid = grid + '<div class="grey_footer rounded_bottom row" style="border-bottom-left-radius: 5px; border-bottom-right-radius: 5px;"></div>';
	} else {
		grid = grid +
//		  '</div>' +
//			'</div>' +
...
function doStyleInjection() {
	...
	if (loc === '/site/partners/dashboard.aspx' ||
	    loc === '/site/account.aspx/settings' ||
		  loc === '/site/partners/admin.aspx') {
		if (activeState) {
